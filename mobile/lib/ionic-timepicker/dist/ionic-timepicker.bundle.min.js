...
function(i) {
    ...
    this.configTimePicker = function(e) { angular.extend(i, e) }, this.$get = ["$rootScope", "$ionicPopup", function(e, t) {
        ...
        function o(i, e) { r.time.hours = i / 3600;
            var t = i % 3600;
        ...
        return r.today = n(new Date).getTime(), r.time = {}, r.increaseHours = function() { r.time.hours = Number(r.time.hours), 12 == r.mainObj.format && (12 != r.time.hours ? r.time.hours += 1 : r.time.hours = 1), 24 == r.mainObj.format && (r.time.hours = (r.time.hours + 1) % 24), r.time.hours = r.time.hours < 10 ? "0" + r.time.hours : r.time.hours }, r.decreaseHours = function() { r.time.hours = Number(r.time.hours), 12 == r.mainObj.format && (r.time.hours > 1 ? r.time.hours -= 1 : r.time.hours = 12), 24 == r.mainObj.format && (r.time.hours = (r.time.hours + 23) % 24), r.time.hours = r.time.hours < 10 ? "0" + r.time.hours : r.time.hours }, r.increaseMinutes = function() { r.time.minutes = Number(r.time.minutes), r.time.minutes = (r.time.minutes + r.mainObj.step) % 60, r.time.minutes = r.time.minutes < 10 ? "0" + r.time.minutes : r.time.minutes }, r.decreaseMinutes = function() { r.time.minutes = Number(r.time.minutes), r.time.minutes = (r.time.minutes + (60 - r.mainObj.step)) % 60, r.time.minutes = r.time.minutes < 10 ? "0" + r.time.minutes : r.time.minutes }, r.changeMeridian = function() { r.time.meridian = "AM" === r.time.meridian ? "PM" : "AM" }, m.openTimePicker = function(e) {
            var n = [];
            r.mainObj = angular.extend({}, i, e), o(r.mainObj.inputTime, r.mainObj.format), n.push({ text: r.mainObj.setLabel, type: "button_set", onTap: function(i) {
                    var e = 0;
                    12 == r.time.format ? (r.time.hours = Number(r.time.hours), "PM" == r.time.meridian && 12 != r.time.hours ? r.time.hours += 12 : "AM" == r.time.meridian && 12 == r.time.hours && (r.time.hours -= 12), e = 60 * r.time.hours * 60 + 60 * r.time.minutes) : e = 60 * r.time.hours * 60 + 60 * r.time.minutes, r.mainObj.callback(e) } }), n.push({ text: r.mainObj.closeLabel, type: "button_close" }), r.popup = t.show({ templateUrl: "ionic-timepicker.html", scope: r, cssClass: "ionic_timepicker_popup", buttons: n }) }, m }] });
